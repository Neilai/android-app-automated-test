__author__ = "Neil"
__time__ = "2018/3/12 18:29"
import os,subprocess
import re,time,signal, psutil
...
def reverseApp(apk):
    ...
    dir=re.search("(.+)\.apk",apk)
    dir=dir[1]
    currentDir=os.getcwd()+"\\"+dir
    des=os.path.join(currentDir,"AndroidManifest.xml")
    data=[]
    with open(des,"r",encoding="utf-8") as f:
        result=f.read()
    result=re.findall(r"(android\.permission\.\w+)",result)
    data=[]
    for item in result:
